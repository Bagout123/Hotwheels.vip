...
public void FindWall(int client)
{
	...
	if (TR_DidHit())
	{
		...
		if (dist <= 100.0)
		{
			...
			/*
			i realise passing through all these params are useless now.. 
			it can be recalculated or the distance check be done in the TryLaunchPlayer function 
			its a waste of resources :/
			*/
			//ask cam if he wants it to scale with distance?
		}
	}
}

/*
public void Test(int client, float fwd[3])
{
